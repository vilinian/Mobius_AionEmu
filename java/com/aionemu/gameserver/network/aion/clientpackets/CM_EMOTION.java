...
/**
 * Handles the {@code CM_EMOTION} packet received from the client.<br>
 * This class processes requests to perform character emotions and broadcasts them to nearby players.<br>
 * It validates the request against the player's current state and permissions.
 * @author SoulKeeper
 * @author_fix nerolory
 */
public class CM_EMOTION extends AionClientPacket
{
	...
	/**
	 * Creates a new instance of the {@link CM_EMOTION} packet.<br>
	 ...
	 * @param restStates Additional optional connection states.
	 */
	...
	protected void runImpl()
	{
		...
		if (player.getLifeStats().isAlreadyDead() || player.getEffectController().isAbnormalState(AbnormalState.CANT_MOVE_STATE2) || player.getEffectController().isUnderFear())
		{
			return;
		}
	...
	/**
	 * Retrieves the unique ID of the target object for a specific {@link Player}.<br>
	 * It checks if the player has an active target first.<br>
	 * If no target exists, it returns the default {@code targetObjectId}.
	 * @param player The {@code Player} object to check.
	 * @return The unique ID of the target or a default value.
	 */
