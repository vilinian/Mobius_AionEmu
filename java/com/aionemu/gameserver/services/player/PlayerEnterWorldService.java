...
/**
 * Handles the logic for a player entering the game world.<br>
 * This service validates entry requirements and initializes necessary data for the {@link Player}.<br>
 * It ensures that all prerequisites are met before allowing the character to join the active session.
 * @author ATracer
 */
public final class PlayerEnterWorldService
{
	...
	private static final String serverIntro = "Please remember: Account sharing is not permitted.";
	...
	/**
	 * Initiates the process for a character to enter the game world.<br>
	 * This method checks for bans and reentry time limits.<br>
	 * It also handles passkey validation if enabled in the configuration.
	 * @param objectId The unique identifier of the character.
	 * @param client The connection object for the player.
	 */
	public static void startEnterWorld(int objectId, AionConnection client)
	{
		...
		final Player edit = playerAccData.getPlayerCommonData().getPlayer();
	...
	/**
	 * Displays the passkey information to the client.<br>
	 * This method updates the {@code ConnectType} and {@code objectId} for the character.<br>
	 * It checks if a passkey exists using {@link PlayerPasskeyDAO}.<br>
	 * Depending on the result, it sends a specific {@code SM_CHARACTER_SELECT} packet.
	 * @param objectId The unique identifier of the game object.
	 * @param client The connection instance for the player.
	 */
	...
	/**
	 * Validates and processes the request to enter a world.<br>
	 * This method prevents duplicate entries for the same {@code objectId}.<br>
	 * It schedules the {@code int)} call with a delay if necessary.
	 * @param objectId The unique identifier of the player character.
	...
	/**
	 * This method handles the logic for a player entering the game world.<br>
	 * It validates the account data and initializes various game services.<br>
	 * The method also sends necessary packets to synchronize the client state.
	 * @param client The {@link AionConnection} object representing the current client connection.
	 * @param objectId The unique identifier of the player character.
	 */
	public static void enterWorld(AionConnection client, int objectId)
	{
		...
		if ((player != null) && client.setActivePlayer(player))
		{
			...
			if (playerAccData.getPlayerCommonData().getLastOnline() != null)
			{
				...
				if (pcd.isReadyForReposteEnergy())
				{
					...
					if (secondsOffline > (4 * 3600))
					{
						...
						// Use player house zones for the additional Energy of Repose bonus.
			...
			// init Quests
			final List<QuestState> questList = new ArrayList<>();
			...
			// SM_TITLE_INFO seems crazy, but this is correct on the official server as of Patch 4.9.1.
			...
			// Send an SM_UNK_154 packet to the client for task 5.6.
			...
			// SM_INVENTORY_INFO, SM_CHANNEL_INFO, SM_STATS_INFO, and SM_CUBE_UPDATE are not on official advancedStigmas.
	...
	// The last stigma was broken at 140001102 and the first stigma was broken at 140000005.
	/**
	 * Removes broken stigma items from the {@code Player}.<br>
	 * This method checks all equipped stigmata for specific item IDs.<br>
	 * It unequips any items that fall within the invalid range.
	 * @param player The {@code Player} object to process.
	 */
	private static void removeBrokenStigmas(Player player)
	{
		final List<Item> stigmaStone = player.getEquipment().getEquippedItemsAllStigma();
	...
	/**
	 * Sends a system message to the player regarding Glory Point deductions.<br>
	 * This method checks the {@code AbyssRankEnum} of the player.<br>
	 * It notifies the player about the specific amount lost based on their rank.
	 * @param player The {@link Player} object receiving the notification.
	 */
	...
	/**
	 * Handles the login logic for players entering the Abyss.<br>
	 * It checks if the {@code Player} has the Supreme Commander rank.<br>
	 * If they do, it sends a system message to all online players.
	 * @param player The {@code Player} object performing the logon.
	 */
	...
	/**
	 * Handles the login logic for a player entering the Abyss Dark zone.<br>
	 * This method checks if the {@code Player} has the Supreme Commander rank.<br>
	 * If they do, it sends a system message to all players in the world.
	 * @param player The {@code Player} object who is logging into the zone.
	 */
	...
	/**
	 * Sends the inventory and item information to the client.<br>
	 * This method calculates storage limits based on player upgrades.<br>
	 * It packages all items into {@code SM_INVENTORY_INFO} packets.<br>
	 * Finally, it sends the latest stats and cube updates.
	 * @param client The {@link AionConnection} used to send packets to the player.
	 * @param player The {@link Player} object containing inventory and equipment data.
	 */
	private static void sendItemInfos(AionConnection client, Player player)
	{
		// Cubesize limit set in inventory.
		final int cubeExpands = player.getCubeExpands();
		...
		client.sendPacket(new SM_INVENTORY_INFO(false, new ArrayList<>(0), cubeExpands, false, player));
	...
	/**
	 * Sends the macro list packet to the connected client.<br>
	 * This method uses {@code SM_MACRO_LIST} to update the player's UI.
	 * @param client The {@link AionConnection} object representing the network connection.
	 * @param player The {@link Player} object containing the character data.
	 */
	...
	/**
	 * Handles the logic for a player successfully logging into the game.<br>
	 * This method updates the online status in the database and initializes world data.<br>
	 * It also triggers the {@code onLoggedIn} event and sets the login time.
	...
	/**
	 * Displays the premium status of an account to the client.<br>
	 * It checks if the {@code Account} has a membership level greater than 0.<br>
	 * If valid, it sends a yellow chat message identifying the account type.
	 * @param client The {@link AionConnection} used to send the packet to the player.
	 * @param account The {@link Account} object containing the membership data.
	 */
