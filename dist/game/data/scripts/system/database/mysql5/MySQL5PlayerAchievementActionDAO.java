...
/**
 * This class provides the {@code MySQL5} database implementation for handling player achievement actions.<br>
 * It manages data persistence for {@link AchievementAction} records within the game database.
 */
public class MySQL5PlayerAchievementActionDAO extends PlayerAchievementActionDAO
{
	private final Logger log = LoggerFactory.getLogger(MySQL5PlayerAchievementActionDAO.class);
	...
	/**
	 * Loads achievement actions from the database for a specific player.<br>
	 * It populates the {@link PlayerAchievement} object with data from the {@code player_achievement_actions} table.
	 * @param player The {@link Player} whose actions need to be loaded.
	 * @param achievement The {@link PlayerAchievement} where the actions will be stored.
	 */
	@Override
	public void loadActions(Player player, PlayerAchievement achievement)
	{
		DB.select(LOAD_QUERY, new ParamReadStH()
		{
			...
			public void handleRead(ResultSet rset) throws SQLException
			{
				while (rset.next())
				{
					final AchievementAction action = new AchievementAction(rset.getInt("object_id"));
	...
	/**
	 * Saves a new {@link AchievementAction} to the database for a specific {@link Player}.<br>
	 * This method executes an insert query using the {@code INSERT_ACTION} string.
	 * @param player The {@link Player} who is performing the action.
	 * @param action The {@link AchievementAction} data to be stored.
	...
	public boolean storeAction(Player player, AchievementAction action)
	{
		...
		try
		{
			...
			final PreparedStatement stmt = con.prepareStatement(INSERT_ACTION);
	...
	/**
	 * Updates the state and step of an achievement action in the database.<br>
	 * This method uses {@code UPDATE_QUERY} to modify existing records.
	 * @param player The {@link Player} object whose ID is used for the update.
	 * @param action The {@link AchievementAction} containing the new data.
	...
	/**
	 * This method removes all records from the database for a specific achievement type.<br>
	 * It uses the {@code AchievementType} provided to filter which actions are deleted.<br>
	 * The operation is performed directly on the {@code player_achievement_actions} table.
	 * @param type The {@code AchievementType} used to identify which records to delete.
	 */
	@Override
	public void deleteAchievementsActions(AchievementType type)
	{
		final PreparedStatement statement = DB.prepareStatement("DELETE FROM player_achievement_actions WHERE type = ?");
		try
		{
	...
	/**
	 * Retrieves all unique object identifiers from the {@code player_achievement_actions} table.<br>
	 * This method queries the database to collect every {@code object_id}.<br>
	 * If an error occurs, it returns an empty array.
	 * @return An array of integers containing the object IDs.
	 */
	@Override
	public int[] getUsedIDs()
	{
		final PreparedStatement statement = DB.prepareStatement("SELECT object_id FROM player_achievement_actions", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
