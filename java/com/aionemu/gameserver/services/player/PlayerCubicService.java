...
/**
 * This service manages the cubic items associated with a {@link Player}.<br>
 * It handles data retrieval, stat calculations, and synchronization of cubic information.<br>
 * It allows for dynamic updates to player stats based on equipped cubics.
 * @author Phantom_KNA
 */
public class PlayerCubicService implements StatOwner
{
	...
	/**
	 * Private constructor for the {@link PlayerCubicService} class.<br>
	 * This prevents other classes from creating new instances of this service.<br>
	 * It logs a message to indicate that the service has been loaded.
	 */
	...
	private final List<IStatFunction> modifiers = new ArrayList<>();
	private final HashMap<Integer, Integer> cubic = new HashMap<>();
	private final HashMap<Integer, Integer> rank = new HashMap<>();
	private final HashMap<Integer, Integer> level = new HashMap<>();
	private final HashMap<Integer, Integer> statValue = new HashMap<>();
	
	/**
	 * Initializes the player's cubic stats and effects upon login.<br>
	 * This method loads monster cubics from the database for the {@code Player}.<br>
	 * It calculates stat modifiers and sends relevant packets to the client.
	...
	public void onLogin(Player player)
	{
		...
		for (PlayerMCEntry playerMonsterCubic : maxMonsterCubic)
		{
			...
			{
				if (cubic.containsKey(cubicId))
				{
					// Enviar los cubus obtenidos por el personaje
					final CubicsTemplate monsterCubic = DataManager.CUBICS_DATA.getCubicsId(cubicId);
					final long itemsCubicInBag = player.getInventory().getItemCountByItemId(monsterCubic.getItemIdCubic());
	 ...
	 */
	/**
	 * Maps a numeric category ID to its corresponding {@link StatEnum}.<br>
	 * This method identifies specific stats based on the provided integer.
	 * @param category The unique identifier for the stat category.
	 * @return The {@code StatEnum} associated with the given category.
	 */
	...
	/**
	 * Registers a cubic item for the specified {@link Player}.<br>
	 * This method updates the player's stats and inventory based on the {@code cubicId}.<br>
	 * It also sends the necessary network packets to sync data with the client.
	 * @param player The {@link Player} who is receiving the cubic.
	 * @param cubicId The unique identifier of the cubic item.
	 */
	...
	/**
	 * Provides the global instance of the {@link PlayerCubicService}.<br>
	 * This method follows the singleton pattern.
	 * @return The active {@code PlayerCubicService} instance.
	 */
