...
/**
 * Handles the Game Master command for applying attribute bonuses to players.<br>
 * This class allows administrators to modify specific stats using {@link StatFunction} logic.<br>
 * It implements {@link StatOwner} to manage the resulting stat changes.
 */
public class CmdAttrBonus extends AbstractGMHandler implements StatOwner
{
	/**
	 * Creates a new instance of the {@code CmdAttrBonus} handler.<br>
	 * This constructor initializes the admin and parameters.<br>
	 * It then automatically executes the {@code run} method.
	...
	/**
	 * Executes the logic to apply an attribute bonus to a player.<br>
	 * It parses the {@code params} string to identify the action and value.<br>
	 * If valid, it adds a {@code StatChangeFunction} to the target's stats and broadcasts a notification.
	 */
	private void run()
	{
