...
/**
 * Handles the admin command to toggle collision for specific game objects.<br>
 * It allows administrators to enable or disable physical interactions in the world.
 * @author Rolandas
 */
public class Collide extends AdminCommand
{
	/**
	 * Initializes the {@link Collide} command.<br>
	...
	/**
	 * Checks for collisions around a selected target and spawns objects at the contact points.<br>
	 * It provides detailed information about nearby geometry to the {@code Player}.
	 * @param admin The {@code Player} who is running the command.
	 * @param params A variable list of strings used to determine the direction of the collision check.
	 */
	@Override
	public void execute(Player admin, String... params)
	{
		...
		{
			final Iterator<CollisionResult> iter = results.iterator();
			int count = 0;
			int closestId = 0;
			String description = "";
			while (iter.hasNext())
			{
				count++;
				final CollisionResult result = iter.next();
		...
		if (params.length == 1)
		{
			...
			{
				final Iterator<CollisionResult> iter2 = results.iterator();
				int count = 0;
				int closestId = 0;
				String description = "";
				...
				{
					count++;
					final CollisionResult result = iter2.next();
		...
		else
		{
			if (closest != null)
			{
				final SpawnTemplate spawn = SpawnEngine.addNewSpawn(admin.getWorldId(), 200000, closest.getContactPoint().x, closest.getContactPoint().y, closest.getContactPoint().z, (byte) 0, 0);
				SpawnEngine.spawnObject(spawn, admin.getInstanceId());
			}
			
			if (closestOpposite != null)
			{
				final SpawnTemplate spawn = SpawnEngine.addNewSpawn(admin.getWorldId(), 200000, closestOpposite.getContactPoint().x, closestOpposite.getContactPoint().y, closestOpposite.getContactPoint().z, (byte) 0, 0);
				SpawnEngine.spawnObject(spawn, admin.getInstanceId());
			}
		}
	}
	...
	{
		final String syntax = "Syntax: //collide [me]";
