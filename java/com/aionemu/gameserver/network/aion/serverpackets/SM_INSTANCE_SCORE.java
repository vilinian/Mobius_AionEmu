...
/**
 * This packet is used to synchronize instance scores with the client.<br>
 * It contains information regarding the current progress and ranking within an instance.
...
public class SM_INSTANCE_SCORE extends AionServerPacket
{
	...
	// Type: 3, 4
	/**
	 * Creates a new {@code SM_INSTANCE_SCORE} object with specific player and reward data.<br>
	 * This constructor initializes the score details for an instance completion.
	 * @param type The type of the instance score.
	 * @param instanceTime The time associated with the instance.
	 * @param instanceReward The {@link InstanceReward} object containing reward information.
	 * @param object The unique identifier for the specific object involved.
	 * @param PlayerStatus The current status of the player.
	 * @param PlayerRaceId The unique ID representing the player's race.
	 */
	...
	// Type: 5, 8, 10, 11
	/**
	 * Creates a new {@code SM_INSTANCE_SCORE} packet.<br>
	 * This constructor initializes the score data for an instance reward.
	 * @param type The type of the instance score.
	 * @param instanceTime The time associated with the instance.
	 * @param instanceReward The {@link InstanceReward} object containing reward details.
	 * @param object The unique identifier for the specific object.
	 */
	...
	/**
	 * Creates a new score object for an instance.<br>
	 * This constructor initializes the score using the provided time, reward, and player list.
	 * @param instanceTime The time associated with the instance.
	 * @param instanceReward The {@link InstanceReward} data used to determine rewards.
	 * @param players A {@code List} of {@link Player} objects involved in the instance.
	 */
	...
	// Type: 6, 7
	/**
	 * Creates a new {@code SM_INSTANCE_SCORE} object to track scores for an instance.<br>
	 * This constructor initializes the score type, time, rewards, and participating players.
	 * @param type The specific type of the instance score.
	 * @param instanceTime The timestamp or duration associated with the instance.
	 * @param instanceReward The {@link InstanceReward} object containing reward details.
	 * @param players A {@code List} of {@link Player} objects involved in the instance.
	 */
	...
	/**
	 * Creates a new {@code SM_INSTANCE_SCORE} object.<br>
	 * This constructor initializes the score with specific reward and type data.
	 * @param instanceReward The {@link InstanceReward} associated with this score.
	 * @param instanceScoreType The {@link InstanceScoreType} category for this score.
	 */
	...
	/**
	 * Creates a new {@code SM_INSTANCE_SCORE} object using the provided reward data.<br>
	 * This constructor initializes the map ID and score type from the {@link InstanceReward}.
	 * @param instanceReward The reward configuration to use for this score.
	 */
	...
	protected void writeImpl(AionConnection con)
	{
		...
		switch (mapId)
		{
			...
			case 301100000: // Training Arena of Unity
				...
				switch (type)
				{
					...
					case 6:
						...
						writeD(0);
						writeD(harmonyArena.getRound());
						final List<HarmonyGroupReward> groups = harmonyArena.getHarmonyGroupInside();
						writeC(groups.size());
						for (HarmonyGroupReward group : groups)
						{
							...
							final List<Player> members = harmonyArena.getPlayersInside(group);
							writeC(members.size());
							int i = 0;
			...
			case 301680000: // Runatorium Ruins
				final RunatoriumReward rr = (RunatoriumReward) instanceReward;
				if (object == null)
				{
					object = ownerObject; // Player ObjectId
				}
				
				final RunatoriumPlayerReward rpr = rr.getPlayerReward(object);
				writeC(type);
				switch (type)
				{
					case 2: // Send 1-Time when "Start_Progress" and "End_Progress"
						final InstanceScoreType isc = rr.getInstanceScoreType();
					...
					case 11: // Send when a NPC/Player has been dead (Send 2-times with different RaceId)
						final int TeamScore = rr.getPointsByRace(rpr.getRace()).intValue();
						final int OppositeTeamScore = rr.getPointsByRace(getOppositeRace(rpr.getRace())).intValue();
						writeC(0);
						writeD(rr.getPvpKillsByRace(rpr.getRace()).intValue());
						writeD(TeamScore);
						writeD(rpr.getRace().getRaceId());
			...
			case 301210000: // Jormungand Marching Route
				final JormungandReward jr = (JormungandReward) instanceReward;
				if (object == null)
				{
					object = ownerObject; // Player ObjectId
				}
				
				final JormungandPlayerReward jpr = jr.getPlayerReward(object);
				writeC(type);
				switch (type)
				{
					case 2:
			...
			case 301670000: // Balaur Marching Route
				final BalaurMarchingRouteReward bmrr = (BalaurMarchingRouteReward) instanceReward;
				if (object == null)
				{
					object = ownerObject; // Player ObjectId
				}
				
				final BalaurMarchingRoutePlayerReward bmrpr = bmrr.getPlayerReward(object);
				writeC(type);
				switch (type)
				{
					case 2:
			...
			case 301220000: // Steel Wall Bastion Battlefield
				final SteelWallBastionBattlefieldReward swbbr = (SteelWallBastionBattlefieldReward) instanceReward;
				if (object == null)
				{
					object = ownerObject; // Player ObjectId
				}
				
				final SteelWallBastionBattlefieldPlayerReward swbbpr = swbbr.getPlayerReward(object);
				writeC(type);
				switch (type)
				{
					case 2:
			...
			case 300320000: // Empyrean Crucible Challenge
				for (CruciblePlayerReward playerReward : (List<CruciblePlayerReward>) instanceReward.getInstanceRewards())
				{
			...
			case 300480000: // Sealed Hall of Knowledge
				final SealedHallOfKnowledgeReward hok = (SealedHallOfKnowledgeReward) instanceReward;
				writeD(hok.getPoints());
				writeD(hok.getNpcKills());
				writeD(0);
				writeD(hok.getRank());
				writeD(hok.getBasicAP());
				if (hok.getPoints() >= 30481)
				{
					// S
			...
			case 300540000: // Steel Wall Bastion
				final SteelWallBastionReward swbr = (SteelWallBastionReward) instanceReward;
				writeD(swbr.getPoints());
				writeD(swbr.getNpcKills());
				writeD(0);
				writeD(swbr.getRank());
				writeD(swbr.getBasicAP());
				if (swbr.getPoints() >= 92000)
				{
					// S
					writeD(186000242);
			...
			case 301400000: // Trillonerk's Gold Vault
				final ShugoEmperorVaultReward shugoReward = (ShugoEmperorVaultReward) instanceReward;
			...
			{
				// Cursed Argent Manor
				final ArgentManorReward argentManorReward = (ArgentManorReward) instanceReward;
			...
			case 301640000:
			{
				...
				final MechanerksWeaponsFactoryReward mechanerksWeaponsFactoryReward = (MechanerksWeaponsFactoryReward) instanceReward;
			...
			case 302000000:
			{
				...
				final FireTempleOfMemoryReward fireTempleReward = (FireTempleOfMemoryReward) instanceReward;
			...
			{
				// Rift of Oblivion
				final RiftOfOblivionReward riftOfOblivionReward = (RiftOfOblivionReward) instanceReward;
				...
				writeD(0);
				writeD(0);
				if ((riftOfOblivionReward.getPoints() >= 23550) || (riftOfOblivionReward.getPoints() >= 21200) || (riftOfOblivionReward.getPoints() >= 17700) || (riftOfOblivionReward.getPoints() >= 14100))
				{
			...
			case 302200000: // Sanctum Battlefield
				final SanctumBattlefieldReward sanctumBattlefieldReward = (SanctumBattlefieldReward) instanceReward;
				final int points1 = sanctumBattlefieldReward.getPoints();
				final int npcKills1 = sanctumBattlefieldReward.getNpcKills();
				final int rank1 = sanctumBattlefieldReward.getRank();
				writeD(points1);
				writeD(npcKills1);
				writeD(rank1);
				break;
			case 302300000: // Pandaemonium Battlefield
				final PandaemoniumBattlefieldReward pandaemoniumBattlefieldReward = (PandaemoniumBattlefieldReward) instanceReward;
				final int points2 = pandaemoniumBattlefieldReward.getPoints();
				final int npcKills2 = pandaemoniumBattlefieldReward.getNpcKills();
				final int rank2 = pandaemoniumBattlefieldReward.getRank();
				writeD(points2);
				writeD(npcKills2);
				writeD(rank2);
				break;
			case 302310000: // Gold Arena TODO
				break;
			case 302320000: // Golden Crucible TODO
				break;
			case 302350000: // Neviwind Canyon
				final NeviwindCanyonReward ncr = (NeviwindCanyonReward) instanceReward;
				if (object == null)
				{
					object = ownerObject; // Player ObjectId
				}
				
				final NeviwindCanyonPlayerReward ncpr = ncr.getPlayerReward(object);
				writeC(type);
				switch (type)
				{
					case 2:
					...
					case 11:
					{
						final int TeamScore = ncr.getPointsByRace(ncpr.getRace()).intValue();
						final int OppositeTeamScore = ncr.getPointsByRace(ncpr.getRace()).intValue();
						writeC(0);
						writeD(ncr.getPvpKillsByRace(ncpr.getRace()).intValue());
						writeD(TeamScore);
						writeD(ncpr.getRace().getRaceId());
	...
	/**
	 * Populates the score table with data for a specific {@link Race}.<br>
	 * This method filters players by their race and writes their rewards to the output.<br>
	 * It handles point calculations based on whether the progress is finished.<br>
	 * If fewer than 6 players are found, it pads the remaining space with null bytes.
	 * @param race The {@link Race} type to filter and process for the table.
	 */
	...
	/**
	 * Determines the opposing faction for a given race.<br>
	 * This method checks the {@code raceId} to return either {@code ELYOS} or {@code ASMODIANS}.
	 * @param race The current {@code Race} object to check.
	 * @return The opposite {@code Race} type.
	 */
	...
	/**
	 * This method populates the Kamar Battlefield reward table for a specific race.<br>
	 * It iterates through players to find those matching the provided {@code race}.<br>
	 * For each matching player, it writes their stats and points to the output stream. If fewer than 12 players are found, it pads the remaining slots with empty data.
	 * @param race The {@code Race} type used to filter which players to include in the table.
	 */
	...
	/**
	 * This method populates the Jormungand reward table for a specific race.<br>
	 * It iterates through players to find those matching the provided {@code race}.<br>
	 * It writes player statistics and rewards into the data stream.<br>
	 * If fewer than 12 players are found, it pads the remaining space with empty bytes.
	 * @param race The {@link Race} type used to filter which players to include in the table.
	 */
	private void JormungandTable(Race race)
	{
		...
		for (Player player : players)
		{
			...
			final JormungandPlayerReward jpr = jr.getPlayerReward(player.getObjectId());
			writeD(player.getObjectId());
			writeC(player.getPlayerClass().getClassId());
			writeC(player.getAbyssRank().getRank().getId());
			writeC(0);
	...
	/**
	 * This method populates the reward table for the Balaur Marching Route.<br>
	 * It filters players based on their {@code Race}.<br>
	 * It writes specific player statistics to the network stream.<br>
	 * It pads the remaining entries to ensure a consistent data length.
	 * @param race The {@code Race} type used to filter which players receive rewards.
	 */
	private void BalaurMarchingRouteTable(Race race)
	{
		...
		for (Player player : players)
		{
			...
			final BalaurMarchingRoutePlayerReward bmrpr = bmrr.getPlayerReward(player.getObjectId());
			writeD(player.getObjectId());
			writeC(player.getPlayerClass().getClassId());
			writeC(player.getAbyssRank().getRank().getId());
			writeC(0);
	...
	/**
	 * This method populates the data for a specific race in the Runatorium reward table.<br>
	 * It iterates through players to find those matching the provided {@code Race}.<br>
	 * It writes player statistics like class ID, abyss rank, and points to the output stream.
	 * @param race The {@code Race} type to filter for during processing.
	 */
	private void DomeTable(Race race)
	{
		...
		for (Player player : players)
		{
			...
			final RunatoriumPlayerReward rpr = rr.getPlayerReward(player.getObjectId());
			writeD(player.getObjectId());
			writeC(player.getPlayerClass().getClassId());
			writeC(player.getAbyssRank().getRank().getId());
			writeC(0); // unk
	...
	/**
	 * This method populates the battlefield table for the Steel Wall Bastion.<br>
	 * It filters players based on their {@code Race}.<br>
	 * It writes specific reward data like kills and points to the network stream.
	 * @param race The {@code Race} type used to filter which players are included in the table.
	 */
	private void SteelWallBastionBattlefieldTable(Race race)
	{
		...
		for (Player player : players)
		{
			...
			final SteelWallBastionBattlefieldPlayerReward swbbpr = swbbr.getPlayerReward(player.getObjectId());
			writeD(player.getObjectId());
			writeC(player.getPlayerClass().getClassId());
			writeC(player.getAbyssRank().getRank().getId());
			writeC(0);
	...
	/**
	 * This method populates the reward table for the Neviwind Canyon instance.<br>
	 * It filters players based on their {@code Race}.<br>
	 * It writes specific player statistics like kills and points to the packet.<br>
	 * It pads the remaining entries with null values to maintain a fixed size.
	 * @param race The {@code Race} type used to filter which players are included in the table.
	 */
	private void NeviwindCanyonTable(Race race)
	{
		...
		for (Player player : players)
		{
			...
			final NeviwindCanyonPlayerReward ncpr = ncr.getPlayerReward(player.getObjectId());
			writeD(player.getObjectId());
			writeC(player.getPlayerClass().getClassId());
			writeC(player.getAbyssRank().getRank().getId());
			writeC(0);
