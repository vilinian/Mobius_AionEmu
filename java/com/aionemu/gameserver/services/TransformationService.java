...
/**
 * Handles the logic for character transformations in the game.<br>
 * This service manages how players change into different forms using {@link ItemTemplate} data.<br>
 * It coordinates between player actions, database updates via {@link AccountTransformDAO}, and network packets.
...
public class TransformationService
{
	...
	/**
	 * Executes a transformation for the specified {@link Player}.<br>
	 * This method processes the item usage and updates the player's transformation list.
	 * @param player The {@link Player} who will perform the transformation.
	 * @param itemObjId The unique object ID of the item used to trigger the transformation.
	 */
	public void makeTransform(Player player, int itemObjId)
	{
		...
		player.getController().addTask(TaskId.ITEM_USE, ThreadPoolManager.getInstance().schedule(() ->
		{
			...
			final AccountTransfo newTrans = new AccountTransfo(transfoRnd, 1);
			player.getTransformCreated().add(newTrans);
			if (player.getTransformList().hasTransformation(newTrans.getCardId()))
			{
				final AccountTransfo transfo = player.getTransformList().getTransformation(newTrans.getCardId());
	...
	/**
	 * Handles the transformation process for a {@link Player}.<br>
	 * This method checks if the player is already transformed.<br>
	 * It consumes an item and triggers the transformation skill.
	 * @param player The {@link Player} who is performing the transformation.
	 * @param itemObjId The unique identifier of the item used to transform.
	 * @param cardId The unique identifier of the transformation card.
	 */
	...
	/**
	 * Handles the process of combining multiple transformations into a new one.<br>
	 * This method checks if the {@code player} has enough Kinah to cover the cost.<br>
	 * It determines the result based on the grade of the first material provided.<br>
	 * The materials are consumed and the new transformation is added to the account.
	 * @param player The {@link Player} who is performing the combination.
	 * @param materials A list of {@code Integer} IDs representing the transformations used as materials.
	 */
	public void onCombineTransformation(Player player, ArrayList<Integer> materials)
	{
		final List<Integer> deleteTransform = new ArrayList<>();
		int kinahCost = 0;
		...
		final int transfoRnd = DataManager.ITEM_TRANSFORM_LIST.getTransformList(list).getTransformId().get(Rnd.get(0, DataManager.ITEM_TRANSFORM_LIST.getTransformList(list).getTransformId().size() - 1));
		if (player.getTransformList().hasTransformation(transfoRnd))
		{
			final AccountTransfo transform = player.getTransformList().getTransformation(transfoRnd);
		...
		for (Integer id2 : materials)
		{
			if (id2 != 0)
			{
				final AccountTransfo transfo = player.getTransformList().getTransformation(id2);
	...
	/**
	 * This method grants all available transformations to a specific {@link Player}.<br>
	 * It iterates through every book in the {@code DataManager}.<br>
	 * Each transformation is added to the player's list and sent as a packet.<br>
	 * The final list of transformations is updated for the player.
	 * @param player The {@code Player} who will receive all transformations.
	 */
	public void addAllToGM(Player player)
	{
		...
		{
			final AccountTransfo transfo = new AccountTransfo(template.getId(), 4);
	...
	/**
	 * Retrieves the single global instance of the {@link TransformationService}.<br>
	 * This method follows the singleton pattern.
	 * @return The active {@code TransformationService} instance.
	 */
