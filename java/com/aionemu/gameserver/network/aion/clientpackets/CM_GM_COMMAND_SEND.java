...
/**
 * This packet handles the transmission of Game Master commands from the client to the server.<br>
 * It processes various administrative actions such as teleporting, changing levels, and managing quests.
...
public class CM_GM_COMMAND_SEND extends AionClientPacket
{
	...
	/**
	 * Sends a GM command packet to the client.<br>
	 * This method initializes the packet with the required states.
	 * @param opcode The unique identifier for the command type.
	 * @param state The primary {@link State} associated with the command.
	...
	protected void readImpl()
	{
		...
		final String clientCmd = readS();
		
		final int index = clientCmd.indexOf(" ");
	...
	protected void runImpl()
	{
		...
		if ((admin == null) || (admin.getAccessLevel() < AdminConfig.GM_PANEL))
		{
			return;
		}
		...
		switch (GmPanelCommands.getValue(cmd))
		{
			...
			case LEVELDOWN:
				...
				break;
			case LEVELUP:
				final QuestState elyos = admin.getQuestStateList().getQuestState(10521);
				final QuestState asmo = admin.getQuestStateList().getQuestState(20521);
