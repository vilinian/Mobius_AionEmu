...
/**
 * Handles the artificial intelligence logic for {@code incarnate} type NPCs during siege events.<br>
...
public class IncarnateAI2 extends AggressiveNpcAI2
{
	...
	{
		super.handleSpawned();
		avatar_scan = ThreadPoolManager.getInstance().scheduleAtFixedRate(() ->
		{
				...
				getOwner().getKnownList().doOnAllPlayers(new Visitor<Player>()
				{
					...
					public void visit(Player player)
					{
						if (player.getAbyssRank().getRank().getId() > AbyssRankEnum.STAR4_OFFICER.getId())
						{
							...
							final EffectController controller = player.getEffectController();
							...
							if (inform)
							{
								final String message = "The power of incarnation removes " + player.getName() + " morph state.";
	 ...
	 * It cancels the {@code avatar_scan} task.<br>
