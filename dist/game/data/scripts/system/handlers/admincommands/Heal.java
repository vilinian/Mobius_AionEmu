...
/**
 * Handles the admin command to restore health to a target.<br>
 * It allows administrators to heal {@link Player} or {@link Creature} objects instantly.
 * @author Mrakobes, Loxo
 */
public class Heal extends AdminCommand
{
	/**
	 * Initializes the {@link Heal} admin command.<br>
	 * This class handles the logic for restoring health to a player.
	 */
	public Heal()
	{
		super("heal");
	...
	/**
	 * Executes the command to heal or refresh a target creature.<br>
	 * It checks if the target is a {@code Creature} and applies various effects based on parameters.<br>
	 * Parameters can specify percentage, raw health values, or specific status refreshes like DP, FP, and energy types.
	 * @param player The admin player executing the command.
	 * @param params Variable arguments for healing amounts or special status flags.
	 */
	...
	public void execute(Player player, String... params)
	{
		...
		else
		{
			...
			try
			{
				...
				final CreatureLifeStats cls = creature.getLifeStats();
				final Pattern heal = Pattern.compile("([^%]+)%");
				final Matcher result = heal.matcher(percent);
				int value;
