...
/**
 * This class handles the asynchronous task of opening a {@link RiftLocation}.<br>
 * It executes the logic required to initialize and activate a rift instance.<br>
 * It is designed to be executed by the {@link ThreadPoolManager} to prevent blocking the main game thread.
 * @author Source
 * @modified CoolyT
 */
public class RiftOpenRunnable implements Runnable
{
	...
	/**
	 * Creates a new task to open a rift.<br>
	 * This constructor initializes the required settings for the {@code run} method.
	 * @param worldId The unique identifier for the game world.
	 * @param guards Whether the rift should spawn with guards set to {@code true}.
	 */
	...
	public void run()
	{
		...
		final String mapName = DataManager.WORLD_MAPS_DATA.getTemplate(worldId).getName();
		final RiftStatistics riftStat = new RiftStatistics();
		final RiftStatistics vortexStat = new RiftStatistics();
		...
		for (RiftLocation loc : locations.values())
		{
			if (loc.getWorldId() == worldId)
			{
				...
				final boolean spawn = Rnd.chance(CustomConfig.RIFT_SPAWNCHANCE);
				...
				final RiftStatistics stat = RiftService.getInstance().openRifts(loc, guards);
