...
/**
 * Handles the {@code info} admin command to display server and game information.<br>
 * This class provides details about the current environment for administrators.
...
{
	/**
	 * Initializes a new instance of the {@link Info} command.<br>
	 * This class handles the admin command for displaying system information.
	 */
	public Info()
	{
		super("info");
	...
	/**
	 * Displays detailed information about the target selected by the administrator.<br>
	 * If the target is a {@code Player}, it shows their stats, tribe, and faction data.<br>
	 * If the target is an {@code Npc}, it shows their attributes, relations, and agro list.
	...
	public void execute(Player admin, String... params)
	{
		...
		if (target instanceof Player)
		{
			...
			final float pvpAttackRatio = player.getGameStats().getStat(StatEnum.PVP_ATTACK_RATIO, 0).getCurrent();
			final float pvpDefenseRatio = player.getGameStats().getStat(StatEnum.PVP_DEFEND_RATIO, 0).getCurrent();
			
			// final float pvpAttack = player.getGameStats().getStat(StatEnum.PVP_ATTACK, 0).getCurrent();
			// final float pvpDefense = player.getGameStats().getStat(StatEnum.PVP_DEFENSE, 0).getCurrent();
			// final float pveAttack = player.getGameStats().getStat(StatEnum.PVE_ATTACK, 0).getCurrent();
			// final float pveDefense = player.getGameStats().getStat(StatEnum.PVE_DEFENSE, 0).getCurrent();
