...
/**
 * Handles the client request to sell broken items.<br>
 * This packet is processed by the {@link TradeService} to exchange items for currency.
...
public class CM_SELL_BROKEN_ITEMS extends AionClientPacket
{
	...
	/**
	 * Handles the packet for selling broken items.<br>
	 * This method initializes the {@link CM_SELL_BROKEN_ITEMS} object with network states.
	...
	protected void readImpl()
	{
		...
		{
			itemId = readD();
			final Item items = player.getInventory().getItemByObjId(itemId);
			count = player.getInventory().getItemCountByItemId(items.getItemId());
			
			final Acquisition aquisition = player.getInventory().getItemByObjId(itemId).getItemTemplate().getAcquisition();
