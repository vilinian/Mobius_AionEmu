...
/**
 * This packet handles the synchronization of items within an event window.<br>
 * It sends data to the client regarding specific items available in a {@link EventsWindow}.
...
public class SM_EVENT_WINDOW_ITEMS extends AionServerPacket
{
	...
	/**
	 * Creates a new {@code SM_EVENT_WINDOW_ITEMS} packet.<br>
	 * This packet contains the list of currently active event windows.<br>
	 * It is used to send event data to the client.
	 * @param active_events_packet The collection of {@link EventsWindow} objects to include in the packet.
	 */
	...
	{
		final int playerAccountId = aionConnection.getActivePlayer().getPlayerAccount().getId();
		...
		{
			final int dbRecivedCount = playerEventsWindowDAO.getRewardRecivedCount(playerAccountId, eventsWindow.getId());
			final int elapsed = playerEventsWindowDAO.getElapsed(playerAccountId, eventsWindow.getId());
			final int displayTime = (eventsWindow.getRemainingTime() - elapsed);
			log.info("event id " + eventsWindow.getId() + " remain " + eventsWindow.getRemainingTime() + " start-time " + (new Timestamp(eventsWindow.getPeriodStart().toInstant().toEpochMilli()).getTime() / 1000) + " end-time " + (new Timestamp(eventsWindow.getPeriodEnd().toInstant().toEpochMilli()).getTime() / 1000) + " total size " + active_events_packet.size());
			...
			writeD(eventsWindow.getMaxCountOfDay()); // This is Max Count of Day
			writeD((int) (eventsWindow.getPeriodStart().toInstant().toEpochMilli() / 1000)); // Period Start TimeStamp
			writeD(0);
			writeD((int) (eventsWindow.getPeriodEnd().toInstant().toEpochMilli() / 1000)); // Period End TimeSTamp
			writeD(0);
