...
/**
 * This class handles the calculation of statistics provided by item enchantments.<br>
 * It extends {@link StatAddFunction} to apply specific bonuses based on an item's enchantment level.
...
public class StatEnchantFunction extends StatAddFunction
{
	...
	/**
	 * Creates a new {@link StatEnchantFunction} for a specific item.<br>
	 * This function calculates stats based on an item's enchantment level.
	 * @param owner The {@code Item} that provides the enchantment bonus.
	 * @param stat The {@code StatEnum} type to be modified.
	 */
	...
	public void apply(Stat2 stat)
	{
		...
		final int enchantLvl = item.getEnchantOrAuthorizeLevel();
		if ((enchantLvl == 0) || ((item.getEquipmentSlot() & ItemSlot.MAIN_OFF_HAND.getSlotIdMask()) != 0) || ((item.getEquipmentSlot() & ItemSlot.SUB_OFF_HAND.getSlotIdMask()) != 0))
		{
			return;
		}
	...
	/**
	 * Calculates the modifier value for an enchantment based on the item type.<br>
	 * It handles specific logic for weapons, accessories, and armor.<br>
	 * Some items double their stats when the level exceeds {@code 20}.
	 * @param enchantLvl The current level of the enchantment.
	 * @param stat The {@link Stat2} object representing the statistic to modify.
	 * @return The calculated modifier value as an {@code int}.
	 */
	...
	/**
	 * Calculates the stat modifiers based on the weapon type and enchantment level.<br>
	 * This method checks the {@code item} template to determine specific multipliers.<br>
	 * It returns a value of {@code 0} if no matching weapon type or stat is found.
	 * @param enchantLvl The current level of the item enchantment.
	 * @return The calculated modifier value as an {@code int}.
	 */
	...
	/**
	 * Calculates the modifier value for accessory items based on their enchantment level.<br>
	 * This method checks specific categories like rings, belts, and bracelets.<br>
	 * It returns different values depending on the {@code autorizeLvl}.
	 * @param autorizeLvl The current enchantment level of the item.
	 * @return The calculated modifier value as an {@code int}.
	 */
	...
	/**
	 * Calculates the armor stat modifiers based on enchantment level and item type.<br>
	 * This method determines specific values for different equipment slots and armor types.<br>
	 * It returns a modifier value that scales with the {@code enchantLvl}.
	 * @param enchantLvl The current level of the item enchantment.
	 * @param applyStat The {@link Stat2} object representing the stat to be modified.
	 * @return The calculated integer modifier for the specified stat.
	 */
				...
				switch (stat)
				{
					...
					case MAGIC_SKILL_BOOST_RESIST: // TODO
						if (item.getEnchantOrAuthorizeLevel() > 20)
						{
							...
							// How it works..
							// After reaching an enchantment level of +21 or higher, Physical Defense and Magic Suppression are added for each additional level.
							return 0;
						}
