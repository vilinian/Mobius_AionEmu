...
/**
 * Handles the request from a client to change an item's equipment slot.<br>
 * This packet processes the logic for moving {@link Item} objects into {@link Equipment} slots for a {@link Player}.
 * @author FrozenKiller
 */
public class CM_CHANGE_EQUIPMENT extends AionClientPacket
{
	...
	private static HashMap<Integer, Integer> itemList = new HashMap<>();
	
	/**
	 * Handles the request to change a player's equipment.<br>
	 * This packet is sent when an item is moved into or out of an equipment slot.<br>
	 * It uses {@link State} to manage the connection status during the transition.
	 * @param opcode The unique identifier for this network operation.
	 * @param state The primary {@code State} of the connection.
	 * @param restStates Variable arguments for additional {@code State} values.
	 */
	...
	protected void runImpl()
	{
		...
		if (itemList.size() != 0)
		{
			...
			{
				final int itemObjectId = entry.getKey();
