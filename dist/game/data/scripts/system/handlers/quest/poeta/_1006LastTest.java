...
/**
 * Handles the final test logic for quest {@code 1006}.<br>
 * This class manages specific player actions and rewards during the concluding stage of the quest.
 * @author FrozenKiller
 */
public class _1006LastTest extends QuestHandler
{
	...
	/**
	 * Initializes the quest handler for the final test.<br>
	 * This method calls the super constructor to set up the {@code questId}.
	 */
			...
			if (targetId == 820006)
			{
				...
				else if (var == 3)
				{
					switch (action)
					{
						...
						case SETPRO4:
							final WorldMapInstance newInstance = InstanceService.getNextAvailableInstance(310020000);
			...
			if (targetId == 820148)
			{
				switch (env.getDialog())
				{
					case SELECTED_QUEST_NOREWARD:
						...
						{
							TeleportService2.teleportTo(player, 210010000, 862.7504f, 1233.8612f, 118.703094f, (byte) 15, TeleportAnimation.BEAM_ANIMATION);
						}
						break;
					default:
						break;
		...
		if ((qs != null) && (qs.getStatus() == QuestStatus.START))
		{
			...
			if (targetId == 211042)
			{
				...
				else if (var == 54)
				{
					qs.setQuestVar(4); // 4
					updateQuestStatus(env);
					final Npc mob = (Npc) QuestService.spawnQuestNpc(310020000, player.getInstanceId(), 211043, (float) 226.7, (float) 251.5, (float) 205.5, (byte) 0);
					mob.getAggroList().addDamage(player, 1000);
					return true;
				}
			}
	...
	/**
	 * Handles logic when a player enters the world for a quest in progress.<br>
	 * This method checks specific quest variables and the current world ID.<br>
	 * It may trigger an ascension morph or send a failure message to the player.
	 * @param env The {@code QuestEnv} containing the current environment data.
	 * @return {@code true} if the ascension morph was triggered, otherwise {@code false}.
	 */
	@Override
