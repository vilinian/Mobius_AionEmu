...
/**
 * Handles the admin command to start a game event.<br>
 * This class processes the logic required to initialize and trigger specific events within the {@link World}.
 */
public class StartEvent extends AdminCommand
{
	/**
	 * Initializes a new instance of the {@link StartEvent} class.<br>
	 * This constructor sets up the command for starting events.
	 */
	public StartEvent()
	{
		super("startevent");
	...
	/**
	 * Executes the command to start a server event and teleport players.<br>
	 * It handles filtering by faction, event status, and specific player conditions.
	 * @param admin The {@code Player} who is running the command.
	 * @param params A variable list of strings containing the event configuration parameters.
	 */
	@Override
	public void execute(Player admin, String... params)
	{
				...
				for (Player p : World.getInstance().getAllPlayers())
				{
					if (Integer.parseInt(params[2]) == 0)
					{
						if (!p.equals(admin))
						{
							...
							PacketSendUtility.sendMessage(p, "You have just been teleported for " + admin.getName() + "'s event.");
						}
					}
					else
					{
						if (p.isLookingForEvent())
						{
							if (!p.equals(admin))
							{
								if (p.getCommonData().getRace().getRaceId() == Integer.parseInt(params[1]))
								{
									...
									PacketSendUtility.sendMessage(p, "You are no longer waiting for an event.");
								}
							}
						}
					}
		...
		PacketSendUtility.sendMessage(admin, playerscount + " players have been teleported");
	}
}
