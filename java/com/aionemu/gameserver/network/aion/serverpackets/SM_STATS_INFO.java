...
/**
 * This packet is used by the server to send user statistics information to the client.<br>
 * It contains data related to {@link PlayerGameStats} and other relevant player stats.
 * @author -Nemesiss-
 * @author Luno
 * @author ginho1
 */
public class SM_STATS_INFO extends AionServerPacket
{
	...
	/**
	 * Creates a new {@code SM_STATS_INFO} packet for a specific player.<br>
	 * This constructor initializes the required statistics data from the {@link Player}.
	 * @param player The {@code Player} object used to gather stats information.
	 */
	...
	protected void writeImpl(AionConnection con)
	{
		...
		{
			final int mainHandminDMG = player.getEquipment().getMainHandWeapon().getItemTemplate().getWeaponStats().getMinDamage();
			final int mainHandmaxDMG = player.getEquipment().getMainHandWeapon().getItemTemplate().getWeaponStats().getMaxDamage();
			final int mainHandfinalDamage = mainHandminDMG + mainHandmaxDMG;
		...
		{
			final int offHandminDMG = player.getEquipment().getOffHandWeapon().getItemTemplate().getWeaponStats().getMinDamage();
			final int offHandmaxDMG = player.getEquipment().getOffHandWeapon().getItemTemplate().getWeaponStats().getMaxDamage();
			final int offHandfinalDamage = offHandminDMG + offHandmaxDMG;
