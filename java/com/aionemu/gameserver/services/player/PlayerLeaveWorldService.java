...
/**
 * Handles the logic for when a {@link Player} disconnects from the game world.<br>
 * It ensures that all active sessions, summons, and temporary states are properly cleaned up or saved.
 * @author ATracer
 */
public class PlayerLeaveWorldService
{
	...
	/**
	 * Starts a delayed process to remove a player from the world.<br>
	 * This method stops the movement of the {@code Player}.<br>
	 * It schedules the {@code startLeaveWorld} method to run after a specific time.
	 * @param player The {@code Player} object to be removed.
	 * @param delay The amount of time in milliseconds to wait before removal.
	 */
	...
	/**
	 * Handles the logout process for a {@code Player}.<br>
	 * It saves all player data, clears active tasks, and updates database records.<br>
	 * This method ensures that the player's state is correctly stored before they disconnect.
	 * @param player The {@code Player} object to log out.
	 */
	public static void startLeaveWorld(Player player)
	{
		if (CustomConfig.ENABLE_STORE_BINDPOINT)
		{
			// Store the binding point before the player logs out (Added by petruknisme).
	...
	/**
	 * Attempts to initiate the world leave process for a {@link Player}.<br>
	 * It stops any current movement and checks if the player is already shutting down.<br>
	 * If not, it starts a delay before leaving the world.
