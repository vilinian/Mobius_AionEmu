...
/**
 * Handles the {@code /ely_channel} player command.<br>
 * This class allows players to switch between different Elyos channels.<br>
 * It validates permissions and updates the player's current world location.
 * @author Maestros
 */
public class cmd_ely_channel extends PlayerCommand
{
	/**
	 * Initializes the {@code ely} channel command.<br>
	 * This constructor sets up the command for use in the game server.
	 */
	...
	/**
	 * Executes a global message command for the Elyos race.<br>
	 * It checks if the {@code player} is an Elyos and not in prison.<br>
	 * The message is sent to all players, other Elyos, or those with admin access.
	 * @param player The {@code Player} executing the command.
	 * @param params Variable arguments containing the message content.
	 */
	@Override
	public void execute(Player player, String... params)
	{
		if ((player.getRace() == Race.ELYOS) && !player.isInPrison())
		{
			...
			final Race adminRace = Race.ELYOS;
