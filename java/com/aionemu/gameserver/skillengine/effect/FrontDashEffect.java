...
/**
 * Handles the logic for a forward dash movement effect.<br>
 * This class calculates and applies the displacement of a {@link Player} in the direction they are facing.
 * @author ATracer
 */
...
public class FrontDashEffect extends EffectTemplate
{
	...
	private final float direction = 0;
	
	/**
	 * Moves the effector of an {@code Effect} to a new position.<br>
	 * This method retrieves the {@link Player} from the effect.<br>
	 * It updates their location based on the coordinates of the associated {@link Skill}.
	 * @param effect The {@code Effect} object containing the movement data.
	 */
	@Override
	...
	public void calculate(Effect effect)
	{
		...
		final float targetZ = GeoService.getInstance().getZ(effector.getWorldId(), effector.getX() + x1, effector.getY() + y1, effector.getZ() + 1.5f, 0.2f, effector.getInstanceId());
		final byte intentions = (byte) (CollisionIntention.PHYSICAL.getId() | CollisionIntention.DOOR.getId());
		final Vector3f closestCollision = GeoService.getInstance().getClosestCollision(effector, effector.getX() + x1, effector.getY() + y1, targetZ, false, intentions);
