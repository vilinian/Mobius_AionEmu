...
/**
 * Handles the admin command to fix broken spawn points in the game world.<br>
 * It identifies and corrects issues with {@link SpawnTemplate} data or active spawns.<br>
 * This utility ensures that NPCs are correctly placed according to the server configuration.
 * @author CoolyT
 */
public class SpawnFix extends AdminCommand
{
	static Collection<SpawnTemplate> spawns = new ArrayList<>();
	static Collection<Npc> walkers = new ArrayList<>();
	static TreeMap<Integer, SpawnGroup2> spawnGroups = new TreeMap<>();
	...
	/**
	 * Initializes the {@link SpawnFix} command.<br>
	...
	/**
	 * Executes the spawn fix command to manage world spawns.<br>
	 * It allows starting or canceling the spawn repair process.
	 * @param admin The {@code Player} who is running the command.
	 * @param params A variable list of strings where the first element must be either "start" or "cancel".
	 */
	@Override
	...
	/**
	 * This method generates an XML file containing spawn data.<br>
	 * It processes the provided {@code SpawnsData2} object and writes it to a new directory.<br>
	 * The output file is named based on the world information.
	 * @param sd The {@code SpawnsData2} object containing the spawn data to be saved.
	 */
	public static void writeXmlFile(SpawnsData2 sd)
	{
		int spawns = 0;
		int spots = 0;
		final StringBuilder template_sb = new StringBuilder();
		...
		final String filename = worldId + "_" + worldName + ".xml";
		try
		{
			File directory;
			...
			final BufferedWriter out = new BufferedWriter(new FileWriter("./data/static_data/spawns/Npcs/New/" + filename));
	...
	/**
	 * Starts a process to fix NPC spawns in the current world.<br>
	 * This method teleports the {@code admin} player to each NPC location.<br>
	 * It deletes existing NPCs and re-spawns them at corrected coordinates.<br>
	 * The process runs on a separate thread to prevent game freezing.
	 * @param admin The {@link Player} who initiated the spawn fix command.
	 */
	public static void go(Player admin)
	{
		...
		thread = ThreadPoolManager.getInstance().schedule(() ->
		{
			...
			try
			{
				for (Npc npc : npcs)
				{
					...
					final WorldPosition pos = npc.getPosition();
					if (pos == null)
					{
					...
					final Spawn sp = spawnData.getSpawnsForNpc(spawn.getWorldId(), spawn.getNpcId());
	...
	/**
	 * Resets the internal state of the {@link SpawnFix} class.<br>
