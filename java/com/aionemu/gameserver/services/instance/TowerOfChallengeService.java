...
/**
 * Manages the logic and mechanics for the Tower of Challenge instance.<br>
 * This service handles player progression, spawning, and related game events within this specific dungeon.
 * @author FrozenKiller
 */
public class TowerOfChallengeService
{
	private final HashMap<Integer, SpawnTemplate> spawns = new HashMap<>();
	
	/**
	 * Moves the {@link Player} to the next floor of the Tower of Challenge.<br>
	 * This method handles specific coordinates and wall spawning based on the current floor level.<br>
	 * It also updates the player's progress packets depending on the entry point.
	 * @param player The {@link Player} object to be teleported.
	 * @param enterFromNpc The ID of the NPC used to enter the tower.
	 */
	public void teleportTowerFloor(Player player, int enterFromNpc)
	{
		final int pfloor = player.getFloor();
		if (pfloor == 0)
		{
	...
	/**
	 * Configures and activates the spawns for a specific floor in the Tower of Challenge.<br>
	 * It populates the {@code spawns} map based on the provided floor number.<br>
	 * This method then calls {@code spawnFloor} to initialize the environment.
	 * @param player The {@code Player} who is entering the new floor.
	 * @param next The floor number to be spawned.
	 */
	private void spawnNextFloor(Player player, int next)
	{
		switch (next)
		{
			...
			case 4: // Silikor of Memory
				final SpawnTemplate spawn1 = SpawnEngine.addNewSingleTimeSpawn(302400000, 247236, 241.05719f, 1249.5562f, 240.63419f, (byte) 60);
			...
			case 11: // TODO
				...
				break;
			case 12: // Marabata
				final SpawnTemplate spawn2 = SpawnEngine.addNewSingleTimeSpawn(302400000, 247238, 229.59697f, 1248.6145f, 240.63419f, (byte) 0);
			...
			case 19:
				...
				break;
			case 20: // Kadena
				final SpawnTemplate spawn3 = SpawnEngine.addNewSingleTimeSpawn(302400000, 247240, 241.10446f, 1249.5867f, 240.63419f, (byte) 60);
			...
			case 23:
				...
				break;
			case 24: // Keltra
				final SpawnTemplate spawn4 = SpawnEngine.addNewSingleTimeSpawn(302400000, 247241, 241.20648f, 1249.4929f, 240.63419f, (byte) 60);
			...
			case 28: // Bollvig Black Heart
				final SpawnTemplate spawn5 = SpawnEngine.addNewSingleTimeSpawn(302400000, 247242, 241.14766f, 1249.5754f, 240.6342f, (byte) 60);
				SpawnEngine.spawnObject(spawn5, player.getPosition().getInstanceId());
				break;
			case 29:
				final SpawnTemplate spawn6 = SpawnEngine.addNewSingleTimeSpawn(302400000, 247289, 1240.9298f, 249.40704f, 240.63698f, (byte) 60); // IDInfinity_Normal_29_01
				SpawnEngine.spawnObject(spawn6, player.getPosition().getInstanceId());
				break;
			case 30:
				final SpawnTemplate spawn7 = SpawnEngine.addNewSingleTimeSpawn(302400000, 247292, 1241.0773f, 1249.4753f, 241.02435f, (byte) 60); // IDInfinity_Normal_30_01
			...
			case 32: // Brigade General Vasharti
				final SpawnTemplate spawn8 = SpawnEngine.addNewSingleTimeSpawn(302400000, 247243, 241.03787f, 1249.4756f, 240.6342f, (byte) 60);
			...
			case 35:
				...
				break;
			case 36: // Vanktrist
				final SpawnTemplate spawn9 = SpawnEngine.addNewSingleTimeSpawn(302400000, 247244, 241.15395f, 1249.4897f, 240.63419f, (byte) 60);
			...
			case 40: // Grendal the Witch TODO
				final SpawnTemplate spawn10 = SpawnEngine.addNewSingleTimeSpawn(302400000, 247245, 241.21867f, 249.41464f, 971.14136f, (byte) 60);
	...
	/**
	 * Spawns all objects for the current floor.<br>
	 * It iterates through the {@code spawns} map and uses {@code int)} to create them.<br>
	 * The {@code spawns} map is cleared after all objects are created.
	 * @param player The {@link Player} who is entering the floor.
	 */
	private void spawnFloor(Player player)
	{
		for (Entry<Integer, SpawnTemplate> floor : spawns.entrySet())
		{
			final SpawnTemplate spawn = floor.getValue();
	...
	/**
	 * Spawns a specific wall object based on the current level.<br>
	 * This method uses {@link SpawnEngine} to place the correct static ID at the player's location.
	 * @param player The {@code Player} who triggered the spawn.
	 * @param level The integer representing the floor level for determining which wall to spawn.
	 */
	public void spawnWall(Player player, int level)
	{
		// Static's 56 107 108 284
		switch (level)
		{
			case 1:
				final SpawnTemplate spawnwall1 = SpawnEngine.addNewSingleTimeSpawn(302400000, 247394, 264.6257f, 248.647f, 242.10599f, (byte) 0); // Fense or Wall
				spawnwall1.setStaticId(284);
				SpawnEngine.spawnObject(spawnwall1, player.getPosition().getInstanceId());
				break;
			case 2:
				final SpawnTemplate spawnwall2 = SpawnEngine.addNewSingleTimeSpawn(302400000, 247394, 1264.6257f, 248.647f, 242.10599f, (byte) 0); // Fense or Wall
				spawnwall2.setStaticId(107);
				SpawnEngine.spawnObject(spawnwall2, player.getPosition().getInstanceId());
				break;
			case 3:
				final SpawnTemplate spawnwall3 = SpawnEngine.addNewSingleTimeSpawn(302400000, 247394, 1264.6257f, 1248.647f, 242.10599f, (byte) 0); // Fense or Wall
				spawnwall3.setStaticId(108);
				SpawnEngine.spawnObject(spawnwall3, player.getPosition().getInstanceId());
				break;
			case 4:
				final SpawnTemplate spawnwall4 = SpawnEngine.addNewSingleTimeSpawn(302400000, 247394, 264.6257f, 1248.647f, 242.10599f, (byte) 0); // Fense or Wall
	...
	/**
	 * Spawns the stairs and flame effects for a specific floor.<br>
	 * This method uses {@link SpawnEngine} to create objects based on the provided level.
	 * @param player The {@code Player} who will see the spawned objects.
	 * @param level The integer representing the current floor level.
	 */
	public void spawnSteps(Player player, int level)
	{
		if (level == 1)
		{
			final SpawnTemplate spawnFlame = SpawnEngine.addNewSingleTimeSpawn(302400000, 247395, 279f, 255f, 242f, (byte) 0);
			spawnFlame.setStaticId(57); // Left Blue Flames (before Stair)
			final SpawnTemplate spawnFlame1 = SpawnEngine.addNewSingleTimeSpawn(302400000, 247395, 279f, 253f, 242f, (byte) 0);
			spawnFlame1.setStaticId(60); // Right Blue Flames (before Stair)
			final SpawnTemplate spawnsteps = SpawnEngine.addNewSingleTimeSpawn(302400000, 247395, 279f, 253f, 242f, (byte) 0);
		...
		{
			final SpawnTemplate spawnFlame = SpawnEngine.addNewSingleTimeSpawn(302400000, 247395, 1279f, 255f, 242f, (byte) 0);
			spawnFlame.setStaticId(66); // Left Blue Flames (before Stair)
			final SpawnTemplate spawnFlame1 = SpawnEngine.addNewSingleTimeSpawn(302400000, 247395, 1279f, 253f, 242f, (byte) 0);
			spawnFlame1.setStaticId(67); // Right Blue Flames (before Stair)
			final SpawnTemplate spawnsteps = SpawnEngine.addNewSingleTimeSpawn(302400000, 247395, 1279f, 253f, 242f, (byte) 0);
		...
		{
			final SpawnTemplate spawnFlame = SpawnEngine.addNewSingleTimeSpawn(302400000, 247395, 1279f, 1255f, 242f, (byte) 0);
			spawnFlame.setStaticId(89); // Left Blue Flames (before Stair)
			final SpawnTemplate spawnFlame1 = SpawnEngine.addNewSingleTimeSpawn(302400000, 247395, 1279f, 1253f, 242f, (byte) 0);
			spawnFlame1.setStaticId(90); // Right Blue Flames (before Stair)
			final SpawnTemplate spawnsteps = SpawnEngine.addNewSingleTimeSpawn(302400000, 247395, 1282f, 1253f, 242f, (byte) 0);
		...
		{
			final SpawnTemplate spawnFlame = SpawnEngine.addNewSingleTimeSpawn(302400000, 247395, 279f, 1255f, 242f, (byte) 0);
			spawnFlame.setStaticId(58); // Left Blue Flames (before Stair)
			final SpawnTemplate spawnFlame1 = SpawnEngine.addNewSingleTimeSpawn(302400000, 247395, 279f, 1253f, 242f, (byte) 0);
			spawnFlame1.setStaticId(59); // Right Blue Flames (before Stair)
			final SpawnTemplate spawnsteps = SpawnEngine.addNewSingleTimeSpawn(302400000, 247395, 282f, 1253f, 242f, (byte) 0);
	...
	/**
	 * Sends a specific tower packet to the player.<br>
	 * This method uses {@code sendPacket} to deliver data.
	 * @param player The {@code Player} who will receive the packet.
	 * @param variable A {@code String} representing the specific tower variable.
	 * @param floor The {@code int} value of the current floor level.
	 */
	private void sendPacket(Player player, String variable, int floor)
	{
		PacketSendUtility.sendPacket(player, new SM_TOWER_OF_CHALLENGE(player, variable, floor));
	}
	
	/**
	 * Moves a {@link Player} to a specific location on the current map.<br>
	 * This method uses {@code int, int, float, float, float, byte)} to update the position.
	...
	/**
	 * Gets the singleton instance of the {@link TowerOfChallengeService}.<br>
	 * Use this method to access the service from anywhere in the code.
	 * @return The single shared instance of {@code TowerOfChallengeService}.
	 */
