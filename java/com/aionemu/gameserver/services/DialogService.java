...
/**
 * This service manages the interaction between players and {@link Npc} entities.<br>
 * It handles the processing of dialogue windows, quest interactions, and related NPC responses.
 * @author VladimirZ
...
{
	/**
	 * Closes the active dialog window for a specific {@link Player}.<br>
	 * This method sends an {@code SM_DIALOG_WINDOW} packet to clear the UI.<br>
	 * It also handles special logic for clearing warehouse users when closing certain NPCs.
	 * @param npc The {@link Npc} that the player is currently interacting with.
	 * @param player The {@link Player} who is closing the dialog.
	 */
	...
	/**
	 * Handles the logic when a player selects an option in a dialog window.<br>
	 * This method processes quest progression and specific NPC interactions.<br>
	 * It also checks for requirements like inventory space or specific items.
	 * @param dialogId The unique identifier of the current dialog.
	 * @param player The {@link Player} who interacted with the NPC.
	 * @param npc The {@link Npc} providing the dialog.
	 * @param questId The ID of the quest being performed.
	...
	public static void onDialogSelect(int dialogId, Player player, Npc npc, int questId, int extendedRewardIndex)
	{
		...
		switch (dialogId)
		{
			case 2:
			{
				...
				{
					// You may be unable to use or equip some items in your purchase list depending on your Abyss Rank, and your rank may decrease; are you sure you want to buy them?
			...
			case 26:
			{
				...
				switch (titleId)
				{
					...
					case 357338: // Belsal Warehouse Manager Aspida
					...
					case 357418: // Aspal Warehouse Manager Atanatos
					...
					case 357498: // Ataj Warehouse Manager Disillon
			...
			case 128:
			{
				...
				if (player.getCommonData().getExpRecoverable() > 0)
				{
					final boolean result = player.getResponseRequester().putRequest(SM_QUESTION_WINDOW.STR_ASK_RECOVER_EXPERIENCE2, responseHandler);
					if (result)
					{
			...
			case 46:
			{
				// 4.3 Learn Craft Improve Extraction
				if (level < 10)
				{
			...
			case 103:
			{
				// 4.3 Purchase List 4.3
				final TradeListTemplate tradeListTemplate = DataManager.TRADE_LIST_DATA.getPurchaseTemplate(npc.getNpcId());
				if (tradeListTemplate == null)
				{
